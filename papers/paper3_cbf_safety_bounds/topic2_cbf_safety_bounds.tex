...
\section{Postscript: Multi-World, Multi-Scale Safety Validation}
The single-world, single-policy observation reported above motivated a
broader validation of the underlying unicycle CBF/QP safety filter itself,
independent of policy competence. The filter (separate obstacle and
inter-agent safety radii, $d_{\text{safe,obs}}=0.22\,$m,
$d_{\text{safe,agent}}=0.45\,$m, solved via OSQP at each control step) was
evaluated across all five benchmark worlds (\texttt{cafe},
\texttt{warehouse}, \texttt{depot}, \texttt{office}, \texttt{maze}) under
the Frontier Heuristic controller, at swarm sizes $N \in \{2,3,5,8\}$, and
separately under two dynamic (non-static) hazard scenarios.

\begin{table}[h]
\centering
\caption{Inter-agent collision count across swarm-size sweep, 200-step episodes, 20 configurations.}
\label{tab:cbf-scaling}
\begin{tabular}{lcccc}
\hline
World & $N{=}2$ & $N{=}3$ & $N{=}5$ & $N{=}8$ \\
\hline
cafe      & 0 & 0 & 2 & 8 \\
warehouse & 0 & 0 & 0 & 0 \\
depot     & 0 & 0 & 0 & 0 \\
...
Inter-agent collisions remained at zero in 18 of 20 swarm-size
configurations, with isolated contacts appearing only at $N{=}8$ in the two
smallest arenas (\texttt{cafe}, $17\,{\times}\,8$\,m; \texttt{office}
corridors), consistent with the filter's safety radius becoming a larger
fraction of available free space as agent density increases, rather than
with any breakdown of the filter's guarantee. Under both dynamic-hazard
tests -- a head-on closing trajectory at $v_{\text{rel}}=0.38\,$m/s and an
orthogonal crossing at $v_{\text{rel}}=0.22\,$m/s -- the filter produced
active braking and standstill-yielding behaviour respectively, holding
minimum clearance above the configured safety radius in both cases with
zero contacts. Separately, root-cause analysis of a non-zero inter-agent
collision count observed in the \texttt{depot} world (6 contacts, 3
consecutive control steps) traced the cause to sensor occlusion: two agents
rounding opposite faces of the same convex structural pillar lost
line-of-sight to each other until already inside the safety radius,
after which the filter's repulsion term separated them within the same
control cycle. This is a sensing-visibility limitation of a purely
reactive, single-step-lookahead filter, not a solver or constraint-encoding
defect, and is a natural target for a longer-horizon or map-aware
extension of the filter presented here.
