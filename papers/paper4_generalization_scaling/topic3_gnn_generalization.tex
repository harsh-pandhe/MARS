...
\section{Postscript: Five-World Empirical Extension}
The two-point topology comparison above was extended to a five-world,
four-swarm-size empirical sweep using the non-learned Frontier Heuristic
controller (Voronoi partitioning plus A* waypoint routing), which does not
require retraining per configuration and therefore permits a much denser
grid over topology and team size than a learned policy would. Table
\ref{tab:topo-sweep} reports final area coverage rate (ACR) at a fixed
200-step horizon across all twenty (world, $N$) configurations.

\begin{table}[h]
\centering
\caption{Area coverage rate (\%), 200-step horizon, by world and swarm size.}
\label{tab:topo-sweep}
\begin{tabular}{lcccc}
\hline
World & $N{=}2$ & $N{=}3$ & $N{=}5$ & $N{=}8$ \\
\hline
cafe      & 64.3 & 69.3 & 66.9 & 98.6 \\
warehouse & 3.4  & 4.8  & 5.3  & 8.7 \\
depot     & 16.9 & 16.6 & 31.9 & 54.4 \\
office    & 5.9  & 6.1  & 12.7 & 46.8 \\
maze      & 1.1  & 3.2  & 6.0  & 10.0 \\
\hline
\end{tabular}
\end{table}

Two findings extend the original two-point observation. First, the
coverage-topology relationship is not monotonic in team size alone: office
and depot both show a super-linear jump between $N{=}5$ and $N{=}8$
(office: $12.7\%\to46.8\%$, a $3.7\times$ increase) consistent with a
partitioning controller crossing a threshold at which concurrent coverage
of disjoint rooms/aisles becomes possible, rather than a smooth scaling
curve. Second, extending the evaluation horizon per world to its
step-starvation-free ceiling (determined by running each world until ACR
plateaus) shows that the 200-step ranking above is itself a step-starved
snapshot: at ceiling, \texttt{office} reaches 98.3\% and \texttt{depot}
reaches 85.7\%, closing most of the apparent topology gap with
\texttt{cafe} (56.0\% ceiling, furniture-bound) and leaving
\texttt{warehouse} (100.0\% ceiling, open-plan) and \texttt{maze} (17.1\%
ceiling, $\approx$83\% solid masonry by area) as the true topological
extremes. This confirms the paper's central claim under a substantially
larger sample of environments: a coverage number is only interpretable
jointly with the topology's reachable-area ceiling and the evaluation
horizon relative to that ceiling, not as a property of the policy alone.
