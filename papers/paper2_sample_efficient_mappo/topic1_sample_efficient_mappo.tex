...
The completed policy was then evaluated against Random Walk and the
classical Frontier Heuristic baseline across 50 episodes (10 per condition)
in the \texttt{cafe} world, and subsequently across all five benchmark
worlds (\texttt{cafe}, \texttt{warehouse}, \texttt{depot}, \texttt{office},
\texttt{maze}) at $N{=}3$ robots. Table~\ref{tab:postscript-results}
reports the outcome.

\begin{table}[h]
\centering
\caption{MAPPO (45 iterations) vs.\ Frontier Heuristic, median ACR (\%), 10-episode evaluation, 150-step episodes.}
...
close the gap to the heuristic in any of the five environments. Root-cause
...
value-loss instability as intended, but did not overcome a deeper
penalty-aversion effect: dense per-step collision penalties dominate the
sparse cell-discovery bonus early in training, and the policy converges to
a low-risk, low-reward equilibrium (near-zero velocity commands) before
exploration pressure can shift it off that equilibrium. We report this as a
completed negative result. The correction proposed in this paper is
necessary to obtain a numerically stable training run at all under a small
sample budget, but is not sufficient to produce a policy competitive with a
classical planner on this task; the dominant remaining failure mode is a
reward-structure problem, not an optimizer or sample-efficiency problem,
