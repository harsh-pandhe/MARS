...
\section{Postscript: Completed Empirical Validation}
The infrastructure fault reported above (a Ray GCS crash triggered by a
host Wi-Fi disassociation event severing the local gRPC control plane) was
subsequently root-caused and fixed by binding Ray strictly to the loopback
interface and capping the Plasma object store in-process rather than via an
ineffective environment variable. Training was resumed and completed to the
full 45-iteration budget without further infrastructure failures.

The completed policy was evaluated under two separate protocols that we
report separately rather than merge, since they differ in sample size and
should not be treated as numerically interchangeable. \textbf{Protocol
A} (10 episodes per condition, \texttt{cafe} only, 50 episodes total)
gives median ACR of 38.6\% (Frontier Heuristic) versus 14.5\% (MAPPO
nominal). \textbf{Protocol B} (single seed, one episode per condition,
all five benchmark worlds at $N{=}3$) gives Table~\ref{tab:postscript-results}.

\begin{table}[h]
\centering
\caption{Protocol B: MAPPO (45 iterations) vs.\ Frontier Heuristic, ACR (\%), single-seed, 150-step episodes, all five worlds.}
\label{tab:postscript-results}
\begin{tabular}{lcc}
\hline
World & Frontier Heuristic & MAPPO (Nominal) \\
\hline
cafe      & 57.0 & 28.2 \\
warehouse & 4.3  & 3.3 \\
depot     & 13.3 & 6.7 \\
...
Contrary to the correction's intended effect, the trained policy did not
close the gap to the heuristic under either protocol, in any of the five
environments evaluated. Root-cause
analysis of the trained policy's rollout behaviour (average displacement of
$1.5\,$m over a 150-step episode, versus $5.4\,$m for the heuristic)
indicates that the reward-shaping correction reduced entropy collapse and
value-loss instability as intended, but did not overcome the policy's
convergence to a low-displacement, low-reward equilibrium (near-zero
velocity commands). This is consistent with dense per-step collision
penalties dominating the sparse cell-discovery bonus, but we did not run a
reward-coefficient ablation to establish this as the causal mechanism
rather than a correlated symptom, nor a longer training budget to rule out
undertraining; both are identified as priority follow-up experiments. We
report this as a completed negative result under the training budget and
reward specified in this paper. The correction proposed in this paper is
necessary to obtain a numerically stable training run at all under a small
sample budget, but is not, on this evidence, sufficient to produce a policy
competitive with a classical planner on this task; the dominant remaining
failure mode is more consistent with a reward-structure problem than an
optimizer or sample-efficiency problem,
and is addressed as a separate line of work.
